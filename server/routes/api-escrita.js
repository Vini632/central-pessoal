...
function handleAI(req, res) {
  ...
  readBody(req).then(({ prompt, currentText }) => {
...
function handleCreate(req, res) {
  readBody(req).then(({ path: filePath }) => {
    ...
    if (!ALLOWED_WRITE.includes(first)) { res.writeHead(403); res.end(JSON.stringify({ error: 'Só é permitido criar em capitulos, cenas ou rascunhos' })); return; }
    const resolved = path.resolve(BOOK_DIR, filePath);
    if (!resolved.startsWith(BOOK_DIR)) { res.writeHead(403); res.end(JSON.stringify({ error: 'Fora do diretório permitido' })); return; }
...
function handleRename(req, res) {
  readBody(req).then(({ oldPath, newPath }) => {
    ...
    if (!oldResolved.startsWith(BOOK_DIR) || !newResolved.startsWith(BOOK_DIR)) { res.writeHead(403); res.end(JSON.stringify({ error: 'Fora do diretório permitido' })); return; }
...
function handleDelete(req, res) {
  readBody(req).then(({ path: filePath }) => {
    if (!filePath) { res.writeHead(400); res.end(JSON.stringify({ error: 'path obrigatório' })); return; }
    const first = filePath.split(/[/\\]/)[0];
    if (!ALLOWED_WRITE.includes(first)) { res.writeHead(403); res.end(JSON.stringify({ error: 'Só é permitido deletar em capitulos, cenas ou rascunhos' })); return; }
    const resolved = path.resolve(BOOK_DIR, filePath);
    if (!resolved.startsWith(BOOK_DIR)) { res.writeHead(403); res.end(JSON.stringify({ error: 'Fora do diretório permitido' })); return; }
...
function handleTreeReadWrite(req, res, url) {
  ...
  if (req.method === 'POST') {
    readBody(req).then(({ path: filePath, content }) => {
      if (!filePath) { res.writeHead(400); res.end(JSON.stringify({ error: 'path obrigatório' })); return; }
      const first = filePath.split(/[/\\]/)[0];
      if (!ALLOWED_WRITE.includes(first)) { res.writeHead(403); res.end(JSON.stringify({ error: 'Só é permitido escrever em capitulos, cenas ou rascunhos' })); return; }
      const resolved = path.resolve(BOOK_DIR, filePath);
      if (!resolved.startsWith(BOOK_DIR)) {
        res.writeHead(403); res.end(JSON.stringify({ error: 'Fora do diretório permitido' }));
        return;
...
function handle(req, res, url) {
  if (url === '/api/escrita/ai') {
    if (req.method !== 'POST') { res.writeHead(405); res.end(JSON.stringify({ error: 'Método não permitido' })); return true; }
    handleAI(req, res);
    return true;
  }
