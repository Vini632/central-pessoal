...
describe('Server Modules', () => {
  ...
  describe('db.js', () => {
    ...
    it('should create all 9 expected tables', () => {
      ...
      for (let i = 0; i < expected.length; i++) {
        assert.strictEqual(names[i], expected[i], `table mismatch at index ${i}: expected ${expected[i]}, got ${names[i]}`);
      }
    });
  ...
  describe('middleware.js', () => {
    ...
    it('checkRateLimit should pass under normal load', () => {
      const res = { writeHead: () => { throw new Error('should not rate limit'); }, end: () => {} };
    ...
    it('checkRateLimit should block when exceeded', () => {
      ...
      let blocked = false;
      const res = {
        writeHead: (code, h) => { blocked = true; assert.strictEqual(code, 429); },
        end: (d) => { const parsed = JSON.parse(d); assert.strictEqual(parsed.error, 'Muitas requisições. Aguarde.'); },
      };
  ...
  describe('api-data route', () => {
    ...
    it('should reject non-GET/POST methods', () => {
      const mod = require('../server/routes/api-data');
      let called = false;
      const res = { writeHead: () => { called = true; }, end: () => {} };
  ...
  describe('api-ollama route', () => {
    it('GET /api/ollama/status should return running status', (t) => {
  ...
  describe('api-ai route', () => {
    it('GET /api/ai/instructions should return content string', (t) => {
  ...
  describe('static route', () => {
    it('should serve index.html for /', (t) => {
      const mod = require('../server/routes/static');
      let statusCode = 0, data = '';
      const res = {
        writeHead: (code) => { statusCode = code; },
        end: (d) => { data = d; },
      };
    ...
    it('should return 404 for nonexistent files', (t) => {
      const mod = require('../server/routes/static');
      let statusCode = 0;
      const res = {
        writeHead: (code) => { statusCode = code; },
  ...
  describe('Middleware without API_TOKEN', () => {
    ...
    it('requireAuth should pass when no API_TOKEN (dev mode)', () => {
      ...
      const res = { writeHead: () => { writeHeadCalled = true; }, end: () => {} };
      const req = { headers: {} };
