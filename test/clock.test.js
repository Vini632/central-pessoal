...
function setupClock(extraMocks = {}) {
  ...
  Clock.init = function() { this.update(); };
...
describe('Clock Module - renderForecast', () => {
      ...
      document: {
        getElementById: (id) => {
          if (id === 'weather-forecast') {
            return {
              _html: '',
              set innerHTML(v) { containerHtml = v || ''; containerHtml = v; },
  ...
  it('shows weekday names in Portuguese abbreviation', () => {
    ...
    assert.ok(weekdays.some(d => containerHtml.includes(d)),
...
describe('Clock Module - fetchWeather all fallbacks fail', () => {
  ...
  beforeEach(() => {
    domTemp = { textContent: '' };
    domDesc = { textContent: '' };
    domForecast = { _html: '', set innerHTML(v) { this._html = v || ''; }, get innerHTML() { return this._html; } };
