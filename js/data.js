...
const Modal = {
  confirm(msg) {
    return new Promise((resolve) => {
      ...
      document.getElementById("modal-body").innerHTML =
        `<p style="color:var(--text-secondary);line-height:1.6">${msg.replace(/</g, "&lt;")}</p>`;
  ...
  prompt(msg, defaultValue = "") {
    return new Promise((resolve) => {
      ...
      document.getElementById("modal-body").innerHTML =
        `<input id="modal-prompt-input" class="settings-select" value="${defaultValue.replace(/"/g, "&quot;")}" style="background-image:none;width:100%">`;
  ...
  async load() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k.startsWith("central_")) {
        try {
          localKeys[k] = JSON.parse(localStorage.getItem(k));
        } catch (e) {
          console.warn("Data.load: localStorage parse error", e);
        }
      }
    }
