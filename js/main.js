document.addEventListener("DOMContentLoaded", () => {
  ...
  function initModules() {
    ...
    try {
      Central.Settings.init();
    } catch (e) {
      console.error("Init error: Settings", e);
    }
    try {
      Central.Clock.init();
    } catch (e) {
      console.error("Init error: Clock", e);
    }
    ...
    function doSearch(query) {
      ...
      if (noteHits.length)
        groups.push({
          label: "Notas",
          module: "notes",
          items: noteHits.map((n) => ({ text: n.content.substring(0, 80), id: n.id })),
        });
      ...
      if (todoHits.length)
        groups.push({
          label: "Tarefas",
          module: "todo",
          items: todoHits.map((t) => ({ text: t.text.substring(0, 80), id: t.id })),
        });
      ...
      const linkHits = links
        ...
        .slice(0, 5);
      if (linkHits.length)
        groups.push({ label: "Links", module: "links", items: linkHits.map((l) => ({ text: l.title, id: l.id })) });
  ...
  function addRippleEffect() {
    const interactiveElements = document.querySelectorAll(
      ".menu-card, .nav-btn, .btn-primary, .btn-secondary, .btn-icon",
    );
    interactiveElements.forEach((el) => {
      el.addEventListener("click", function (e) {
        try {
          ...
          /* ripple is cosmetic, fail silently */
        }
      });
    });
