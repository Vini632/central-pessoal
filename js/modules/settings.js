...
const Settings = {
  ...
  save() {
    ...
    if (this._ytKey) {
      apiFetch("/api/settings/youtubeApiKey", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ value: this._ytKey }),
      }).catch(() => {});
    }
    if (this._driveToken) {
      apiFetch("/api/settings/driveToken", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ value: this._driveToken }),
      }).catch(() => {});
    }
  },
  ...
  async open() {
      ...
      <div class="settings-group">
        ...
        <select class="settings-select" id="set-ai-model">
          ${
            models.length
              ? models
                  .map((m) => `<option value="${m}" ${this.data.aiModel === m ? "selected" : ""}>${m}</option>`)
                  .join("")
              : '<option value="">Nenhum modelo disponivel</option>'
          }
        </select>
    ...
    apiFetch("/builds/CentralPessoal.exe", { method: "HEAD" })
      .then((r) => {
        if (r.ok) {
  ...
  async testYtKey() {
    ...
    if (!key) {
      statusEl.textContent = "⚠️ Cole uma chave primeiro.";
      return;
    }
  ...
  driveConnect() {
    ...
    if (!clientId) {
      Toast.warn("Coloque seu Client ID do Google Cloud primeiro.");
      return;
    }
  ...
  async driveBackup() {
    if (!this._driveToken) {
      Toast.warn("Conecte ao Google Drive primeiro.");
      return;
    }
    try {
      const backup = {};
      ...
      const metadata = {
        name: `central-backup-${new Date().toISOString().slice(0, 10)}.json`,
