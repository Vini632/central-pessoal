...
const Calendar = {
  ...
  checkReminders() {
    ...
    for (const ev of this.data) {
      ...
      if (ev.reminderTime === currentMin) {
        ...
        try {
          Data.save("central_notified", [...this.notified]);
        } catch (e) {
          console.warn("calendar: catch", e);
        }
  ...
  openView(dateStr, event) {
    ...
    const locStr = event.location
      ? `<div class="cal-view-row"><span class="cal-view-label">Local</span><span>${event.location}</span></div>`
      : "";
    const descStr = event.description
      ? `<div class="cal-view-row"><span class="cal-view-label">Descricao</span><span>${event.description.replace(/</g, "&lt;")}</span></div>`
      : "";
    const reminderStr = event.reminder
      ? `<div class="cal-view-row"><span class="cal-view-label">Lembrete</span><span>🔔 ${event.reminderTime || "Horario do evento"}</span></div>`
      : "";
