...
const Habits = {
  ...
  init() {
    ...
    if (addBtn)
      addBtn.addEventListener("click", () => {
        console.log("habit-add-btn clicked");
        this.create();
      });
  ...
  create() {
    ...
    if (!name) {
      console.log("Habits.create(): empty, returning");
      return;
    }
  ...
  renderCharts(week) {
    ...
    const bestHabit = this.habits
      .map((h) => ({ name: h.name, icon: h.icon, count: week.filter((w) => this.isDone(h.id, w)).length }))
    ...
    const _maxBar = Math.max(1, ...barData.map((b) => b.done));
