...
const Leitura = {
  ...
  render() {
    ...
    if (this.data.length === 0) {
      container.innerHTML =
        '<div class="leitura-empty"><div style="font-size:40px;margin-bottom:12px">📚</div><p>Nenhum link salvo</p><p style="font-size:12px;margin-top:4px">Cole uma URL acima para começar</p></div>';
      return;
    }
    ...
    `,
      ...
      .join("");
    container
      .querySelectorAll(".leitura-open-btn")
      .forEach((b) => b.addEventListener("click", () => this.open(b.dataset.id)));
    container
      .querySelectorAll(".leitura-toggle-btn")
      .forEach((b) => b.addEventListener("click", () => this.toggle(b.dataset.id)));
    container
      .querySelectorAll(".leitura-del-btn")
      .forEach((b) => b.addEventListener("click", () => this.delete(b.dataset.id)));
