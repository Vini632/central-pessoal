...
const Game = {
  ...
  data: {
    ferreiro: {
      ...
      materials: {
        ferro: { name: "Ferro", price: 4 },
        carvao: { name: "Carvão", price: 2 },
        madeira: { name: "Madeira", price: 1 },
        gemas: { name: "Gemas", price: 15 },
      },
      recipes: [
        ...
        { id: "escudo", name: "Escudo", cost: { ferro: 3, madeira: 1 }, price: 38, xp: 6, minLevel: 2, energy: 2 },
        {
          ...
          cost: { ferro: 3, carvao: 2, madeira: 1 },
          price: 52,
          xp: 8,
          minLevel: 3,
          energy: 3,
        ...
        {
          id: "coroa",
          name: "Coroa Ornamental",
          cost: { gemas: 3, ferro: 2 },
          price: 90,
          xp: 12,
          minLevel: 4,
          energy: 4,
    ...
    bar: {
      ...
      materials: {
        cerveja: { name: "Cerveja", price: 2 },
        vinho: { name: "Vinho", price: 4 },
        destilado: { name: "Destilado", price: 6 },
        petisco: { name: "Petisco", price: 3 },
      ...
      recipes: [
        ...
        {
          id: "taverna",
          name: "Taverna Especial",
          cost: { cerveja: 2, destilado: 1 },
          price: 20,
          xp: 5,
          minLevel: 2,
          energy: 2,
        ...
        },
        {
          id: "feast",
          name: "Festim da Casa",
          cost: { petisco: 2, cerveja: 2, destilado: 1, importado: 1 },
          price: 60,
          xp: 14,
          minLevel: 4,
          energy: 4,
    ...
    mercado: {
      ...
      materials: {
        trigo: { name: "Trigo", price: 2 },
        tecido: { name: "Tecido", price: 5 },
        ferramentas: { name: "Ferramentas", price: 8 },
        livros: { name: "Livros", price: 12 },
      ...
      recipes: [
        ...
        {
          id: "pacote_campones",
          name: "Pacote Camponês",
          cost: { trigo: 2, tecido: 1 },
          price: 20,
          xp: 4,
          minLevel: 2,
          energy: 2,
        ...
        },
        {
          id: "pacote_nobre",
          name: "Pacote Nobre",
          cost: { livros: 1, especiarias: 2, tecido: 1 },
          price: 65,
          xp: 12,
          minLevel: 4,
          energy: 3,
  ...
  save() {
    try {
      Data.save("central_game", this.state);
    } catch (e) {
      console.warn("game: catch", e);
    }
  },
  ...
  newGame() {
    ...
    document.getElementById("modal-body").innerHTML = Object.entries(this.data)
      .map(
        ([k, v]) => `
  ...
  timeStr() {
    return String(this.state.hour).padStart(2, "0") + ":00";
  },
  ...
  getLevelMod() {
    return 1 + (this.state.level - 1) * 0.1;
  },
  ...
  getPrice(recipe) {
    ...
    const vitrineMod =
      this.state.upgrades.includes("vitrine") || this.state.upgrades.includes("vitrine_mercado") ? 1.2 : 1;
  ...
  async doAction(action) {
    ...
    switch (action) {
      case "buy": {
        ...
        document.getElementById("modal-body").innerHTML = Object.entries(cfg.materials)
          .map(
            ([key, mat]) => `
        ...
        const buyHandler = () => {
          ...
          if (total > s.money) {
            Toast.warn("Dinheiro insuficiente!");
            return;
          }
          if (total === 0) {
            overlay.classList.add("hidden");
      ...
      case "work": {
        if (s.energy < 2) {
          this.addLog("⚠️ Sem energia! Descanse (Avançar para 08:00).");
          return;
        }
        const bonus = s.hired.reduce((sum, eid) => {
          const e = cfg.employees.find((ee) => ee.id === eid);
          return sum + (e ? e.bonus : 0);
        }, 0);
      ...
      case "serve": {
        if (s.hour < 6 || s.hour > 22) {
          this.addLog("Estabelecimento fechado.");
          return;
        }
        if (s.energy < 1) {
          this.addLog("⚠️ Sem energia!");
          return;
        }
        const custCount = this.getCustomerCount();
        if (custCount <= 0) {
          this.addLog("Nenhum cliente hoje.");
          s.hour++;
          this.checkEndOfDay();
          return;
        }
        ...
        if (!can) {
          this.addLog(`${customer} pediu ${recipe.name}, mas faltam materiais.`);
          s.hour++;
          this.checkEndOfDay();
          return;
        }
        ...
        this.addLog(
          `${customer} pediu ${recipe.name} → $${price}${tip ? " (+$" + tip + " gorjeta)" : ""} ⭐+${repGain}`,
        );
      ...
      case "rest": {
        ...
        if (restored === 0) {
          this.addLog("Energia já está cheia.");
          return;
        }
      ...
      case "loan": {
        ...
        const takeLoan = (amount, interest) => {
          if (s.loan.owed > 0) {
            Toast.warn("Já tem um empréstimo ativo!");
            return;
          }
          s.money += amount;
  ...
  randomEvent() {
    const s = this.state;
    const _cfg = this.data[s.type];
    const events = [
      {
        text: "Cliente pagou a mais! +$8",
        fn: () => {
          s.money += 8;
          s.reputation += 1;
        },
      ...
      {
        text: "Furtaram mercadorias... -$12",
        fn: () => {
          s.money = Math.max(0, s.money - 12);
          s.reputation = Math.max(0, s.reputation - 2);
        },
      },
      {
        text: "Carregamento extra chegou!",
        fn: () => {
          Object.keys(s.inventory).forEach((k) => (s.inventory[k] = (s.inventory[k] || 0) + 3));
        },
      },
      {
        text: "⭐ Fama cresceu na região! +5",
        fn: () => {
          s.reputation += 5;
        },
      },
      {
        text: "Cliente reclamou no guia. -2 ⭐",
        fn: () => {
          s.reputation = Math.max(0, s.reputation - 2);
      ...
      {
        text: "Um andarilho ofereceu informação. +📈",
        fn: () => {
          s.influence += 2;
        },
      },
      {
        text: "Dia de festival! +50% movimento",
        fn: () => {
          s.money += Math.floor(Math.random() * 20) + 5;
          this.addLog("(movimento extra rendeu grana!)");
        },
      },
      {
        text: "Equipamento quebrou. -$15 reparo",
        fn: () => {
          s.money = Math.max(0, s.money - 15);
        },
      },
      {
        text: "⭐ Cliente influente elogiou! +3",
        fn: () => {
          s.reputation += 3;
          s.influence += 1;
        },
      ...
      {
        text: "💰 Achou uma bolsa com dinheiro! +$20",
        fn: () => {
          s.money += 20;
        },
      },
  ...
  craft(recipeId) {
    ...
    if (!recipe) return;
    if (this.state.level < recipe.minLevel) {
      this.addLog(`Nível ${recipe.minLevel} necessário para ${recipe.name}.`);
      this.render();
      return;
    }
    if (this.state.energy < (recipe.energy || 1)) {
      this.addLog("⚠️ Sem energia!");
      this.render();
      return;
    }
  ...
  hire(employeeId) {
    ...
    if (this.state.money < emp.cost) {
      this.addLog(`Dinheiro insuficiente para contratar ${emp.name}`);
      this.render();
      return;
    }
  ...
  buyUpgrade(upgradeId) {
    ...
    if (this.state.money < upg.cost) {
      this.addLog(`Dinheiro insuficiente para ${upg.name}`);
      this.render();
      return;
    }
  ...
  render() {
    ...
    if (!cfg) {
      section.innerHTML =
        '<div class="module-header"><h2>Jogo</h2></div><div class="game-empty"><p>Erro: dados do jogo corrompidos.</p><button id="game-reset-btn" class="btn-primary" style="margin-top:16px">REINICIAR</button></div>';
      section.querySelector("#game-reset-btn")?.addEventListener("click", () => {
        Data.remove("central_game");
        this.state = null;
        this.render();
      });
      ...
      <div class="game-body">
        <div class="game-left">
          <div class="game-log">
            ...
            <div class="game-log-entries">${s.log
              .slice(-8)
              .map((l) => `<div class="game-log-entry">${l}</div>`)
              .join("")}</div>
          </div>
          <div class="game-tabs">
          ...
          <div class="game-panels">
            ...
            <div class="game-panel" id="game-panel-inventory">
              ...
              ${Object.entries(cfg.materials)
                .map(
                  ([k, m]) =>
                    `<div class="game-inv-row"><span>${m.name}</span><span class="game-inv-qty">${s.inventory[k] || 0}</span><span class="game-inv-price">$${m.price}</span></div>`,
                )
                .join("")}
              <div class="game-panel-title" style="margin-top:12px">Receitas</div>
              ${cfg.recipes
                .filter((r) => r.minLevel <= s.level)
                .map((r) => {
                  ...
                  <span class="game-recipe-cost">${Object.entries(r.cost)
                    .map(([m, q]) => `${cfg.materials[m].name}x${q}`)
                    .join(" ")}</span>
            ...
            <div class="game-panel" id="game-panel-stats">
              ...
              ${
                s.achievements.length === 0
                  ? '<div class="game-empty-small">Nenhuma conquista ainda</div>'
                  : this.achievements
                      .filter((a) => s.achievements.includes(a.id))
                      .map(
                        (a) =>
    ...
    section
      .querySelectorAll(".game-action-btn")
      .forEach((b) => b.addEventListener("click", () => this.doAction(b.dataset.action)));
    section
      .querySelectorAll(".game-craft-btn")
      .forEach((b) => b.addEventListener("click", () => this.craft(b.dataset.recipe)));
    section
      .querySelectorAll(".game-hire-btn")
      .forEach((b) => b.addEventListener("click", () => this.hire(b.dataset.employee)));
    section
      .querySelectorAll(".game-buy-upg-btn")
      .forEach((b) => b.addEventListener("click", () => this.buyUpgrade(b.dataset.upgrade)));
  },
};
