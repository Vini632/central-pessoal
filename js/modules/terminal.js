  ...
  execLocal(cmd) {
    ...
    const cmds = {
      ...
      connect: () => {
        const baseUrl =
          args[0] ||
          `${window.location.protocol === "https:" ? "wss:" : "ws:"}//${window.location.hostname}:${window.location.port || "80"}/terminal`;
        const token = document.querySelector('meta[name="api-token"]')?.content;
        ...
        try {
          ...
          this.ws.onerror = () => {
            this.writeln("Falha ao conectar.");
          };
        } catch (e) {
          this.writeln("Erro: " + e.message);
        }
