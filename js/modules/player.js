const Player = {
  ...
  createSound(id) {
    ...
    switch (id) {
      ...
      case 'rain': {
        ...
        source.connect(bp); bp.connect(output); source.start();
        return { source, output };
      }
      case 'ocean': {
        for (let i = 0; i < bufferSize; i++) data[i] = Math.random() * 2 - 1;
        ...
        const lg2 = ctx.createGain(); lg2.gain.value = 150;
        lfo2.connect(lg2); lg2.connect(lp.frequency); lfo2.start();
        source.connect(lp); lp.connect(output); source.start();
        return { source, output };
      }
      case 'wind': {
        for (let i = 0; i < bufferSize; i++) data[i] = Math.random() * 2 - 1;
        ...
        const lg3 = ctx.createGain(); lg3.gain.value = 600;
        lfo3.connect(lg3); lg3.connect(hp.frequency); lfo3.start();
        source.connect(hp); hp.connect(output); source.start();
        return { source, output };
      }
      case 'brown': {
        let last = 0;
        for (let i = 0; i < bufferSize; i++) { last = (last + (Math.random() * 2 - 1) * 0.02) / 1.02; data[i] = last * 3; }
        source = ctx.createBufferSource(); source.buffer = buffer; source.loop = true;
        source.connect(output); source.start();
        return { source, output };
      }
  ...
  loadURL() {
    ...
    if (!parsed) {
      input.value = ''; input.placeholder = 'URL invalida';
      return;
    }
    ...
    input.value = ''; input.placeholder = 'URL do video ou playlist';
  },
  ...
  saveQueue() {
    try { localStorage.setItem('central_yt_queue', JSON.stringify(this.queue.slice(0, 50))); } catch {}
  },
  ...
  async search() {
    ...
    try {
      ...
      document.getElementById('yt-search-results').innerHTML = data.items.map(item => `
      ...
      document.getElementById('yt-search-results').querySelectorAll('.yt-search-item').forEach(el => {
  ...
  saveHistory() {
    try { localStorage.setItem('central_yt_history', JSON.stringify(this.history)); } catch {}
  },
  ...
  renderHistory() {
    ...
    if (this.history.length === 0) {
      list.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-tertiary)">Nenhum histórico ainda</div>';
      return;
    }
    list.innerHTML = `
  ...
  savePlaylists() {
    try { localStorage.setItem('central_yt_playlists', JSON.stringify(this.playlists)); } catch {}
  },
  ...
  renderPlaylists() {
    ...
    if (this.playlists.length === 0) {
      list.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-tertiary)">Nenhuma playlist criada</div>';
      return;
    }
    ...
    list.querySelectorAll('.yt-pl-play').forEach(b => b.addEventListener('click', () => this.playPlaylist(b.dataset.playlist)));
    list.querySelectorAll('.yt-pl-del').forEach(b => b.addEventListener('click', () => this.deletePlaylist(b.dataset.playlist)));
    list.querySelectorAll('.yt-pl-v-del').forEach(b => b.addEventListener('click', (e) => { e.stopPropagation(); this.removeFromPlaylist(b.dataset.playlist, b.dataset.video); }));
      ...
      b.addEventListener('click', () => {
        ...
        results.forEach(el => {
          ...
          if (!exist) {
            ...
            btn.innerHTML = '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14"/><path d="M5 12h14"/></svg>';
            btn.style.cssText = 'background:none;border:none;color:var(--text-muted);cursor:pointer;padding:4px';
            btn.addEventListener('click', (e) => { e.stopPropagation(); this.addToPlaylist(id, el.dataset.id, el.querySelector('.yt-si-title')?.textContent || el.dataset.id); });
